...
<body>
    <div class="header">
        <h1>Fast Moving Products Report</h1>
    ...
    @if($data['products']->count() > 0)
        <div class="info-box">
            ...
            <p><strong>Total Sales:</strong> Tsh {{ number_format($data['summary']['total_sales'], 2) }}</p>
        </div>
            ...
            <tbody>
                @foreach($data['products'] as $index => $item)
                    <tr>
                        ...
                        <td class="text-right">Tsh {{ number_format($item['total_sales'], 2) }}</td>
                        <td>{{ $item['unit'] }}</td>
                    </tr>
                @endforeach
                <tr class="total-row">
                    ...
                    <td class="text-right"><strong>Tsh {{ number_format($data['summary']['total_sales'], 2) }}</strong></td>
                    <td></td>
                </tr>
            </tbody>
        </table>
    @else
        <div class="info-box">
            <p><strong>No fast moving products found for the selected period.</strong></p>
        </div>
    @endif
